...
A classe TimeStamp tem como atributos a hora, os minutos e os segundos e um método que atualiza os valores dos atributos com a ajuda da classe java.util.Calendar do sistema. A TimeStamp modela a ``lógica de negócio'' de um relógio. A classe Calendar do pacote java.util é uma classe que vem na biblioteca padrão distribuida com o JDK, kit de desenvolvimento do Java. Os controles de acesso dos atributos estão em ``default'' que em Java quer dizer que os atributos estão visíveis para todos dentro do mesmo pacote (diretório de compilação). Se esta não fosse uma aplicação ``didática'', os atributos teriam visibilidade privada e teriamos métodos apenas para leitura destes atributos para todos de fora da classe já que não faz sentido alguém de fora da classe alterar os valores de horas, minutos e segundos.
...
class ClockView extends javax.swing.JFrame {
    ...
    ClockView() {
        setDefaultCloseOperation(javax.swing.WindowsConstants.EXIT_ON_CLOSE);
        setSize(95,45);
    ...
    public void refreshTimeDisplay() {
        ...
        t.fillTimes();
        String display = getDigitAsString(t.hrs) + ":"
                     + getDigitAsString(t.mins) + ":"
                     + getDigitAsString(t.secs);
